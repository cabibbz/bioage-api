...
function resolveContextualQualitativeValue(qualitativeKey: string) {
  ...
  const contextualPatterns = [
    ...
    {
      pattern:
        /^(?:repeat\s+)?(?:screen|screening|qualitative|assay|screening assay|qualitative assay|screening result|qualitative result|result|result status)(?:\s*(?::|=|\/|\||-)\s*|\s*[,;]\s*|\s+)(positive|pos|negative|neg|detected|not[\s-]*detected|none detected|undetected|reactive|non[\s-]*reactive)$/,
      baseGroup: 1,
    },
    {
      pattern:
        /^(?:repeat\s+)?(?:screen|screening|qualitative|assay|screening assay|qualitative assay|screening result|qualitative result|result|result status)\s*,\s*(positive|pos|negative|neg|detected|not[\s-]*detected|none detected|undetected|reactive|non[\s-]*reactive)$/,
      baseGroup: 1,
    },
    {
      pattern:
