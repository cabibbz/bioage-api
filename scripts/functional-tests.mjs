...
function createPrefixProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      valueString: "QUALITATIVE NEGATIVE RESULT",
      effectiveDateTime: "2026-04-03T09:50:00.000Z",
    }),
  );
}

function createResultLabeledProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      effectiveDateTime: "2026-04-03T09:55:00.000Z",
    }),
  );
}

function createParentheticalProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
...
function createColonProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      valueString: "SCREENING: POSITIVE",
      effectiveDateTime: "2026-04-03T10:05:00.000Z",
    }),
  );
}

function createCommaLabeledProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      valueString: "SCREENING RESULT, POSITIVE",
      effectiveDateTime: "2026-04-03T10:10:00.000Z",
    }),
  );
}

function createSuffixProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      valueString: "POSITIVE SCREENING RESULT",
      effectiveDateTime: "2026-04-03T10:15:00.000Z",
    }),
  );
}

function createBaseLabeledProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      valueString: "POSITIVE: SCREENING RESULT",
      effectiveDateTime: "2026-04-03T10:20:00.000Z",
    }),
  );
}
...
function createSemicolonSeparatedProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      valueString: "SCREENING RESULT; POSITIVE",
      effectiveDateTime: "2026-04-03T10:30:00.000Z",
    }),
  );
}

function createPipeSeparatedProcessTextFhirResource() {
  return Buffer.from(
    JSON.stringify({
      resourceType: "Observation",
      status: "final",
      ...
      valueString: "SCREENING RESULT | POSITIVE",
      effectiveDateTime: "2026-04-03T10:35:00.000Z",
    }),
  );
}
...
const scenarios = [
  ...
  {
    name: "report-intake-normalizes-prefix-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional prefix-context panel",
        observedAt: "2026-04-04T11:50:00.000Z",
        entries: [
          { name: "CRP", textValue: "SCREENING POSITIVE" },
          { name: "Apolipoprotein B", textValue: "QUALITATIVE NEGATIVE RESULT" },
          { name: "Glucose", textValue: "ASSAY NON REACTIVE" },
        ],
      });
      ...
      assert.ok(crpMeasurement.note.includes('from "SCREENING POSITIVE" to "positive"'));

      const apobMeasurement = report.measurements.find((measurement) => measurement.canonicalCode === "apob");
      assert.ok(apobMeasurement);
      assert.equal(apobMeasurement.textValue, "negative");
      assert.ok(apobMeasurement.note.includes('from "QUALITATIVE NEGATIVE RESULT" to "negative"'));

      const glucoseMeasurement = report.measurements.find((measurement) => measurement.canonicalCode === "fasting_glucose");
      assert.ok(glucoseMeasurement);
      assert.equal(glucoseMeasurement.textValue, "non-reactive");
      assert.ok(glucoseMeasurement.note.includes('from "ASSAY NON REACTIVE" to "non-reactive"'));
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T11:50:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T11:50:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T11:50:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-result-labeled-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional result-labeled panel",
        observedAt: "2026-04-04T11:55:00.000Z",
        entries: [
          { name: "CRP", textValue: "SCREENING RESULT: POSITIVE" },
          { name: "Apolipoprotein B", textValue: "QUALITATIVE ASSAY RESULT NEGATIVE" },
      ...
      assert.ok(apobMeasurement);
      assert.equal(apobMeasurement.textValue, "negative");
      assert.ok(apobMeasurement.note.includes('from "QUALITATIVE ASSAY RESULT NEGATIVE" to "negative"'));
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T11:55:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T11:55:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T11:55:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-parenthetical-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional parenthetical panel",
        observedAt: "2026-04-04T12:00:00.000Z",
        entries: [
          { name: "CRP", textValue: "POSITIVE (SCREENING RESULT)" },
          { name: "Apolipoprotein B", textValue: "QUALITATIVE ASSAY (NEGATIVE)" },
          { name: "Glucose", textValue: "REPEAT SCREEN (NON REACTIVE)" },
        ],
      });
      ...
      assert.ok(crpMeasurement);
      assert.equal(crpMeasurement.textValue, "positive");
      assert.ok(crpMeasurement.note.includes('from "POSITIVE (SCREENING RESULT)" to "positive"'));

      const apobMeasurement = report.measurements.find((measurement) => measurement.canonicalCode === "apob");
      assert.ok(apobMeasurement);
      assert.equal(apobMeasurement.textValue, "negative");
      assert.ok(apobMeasurement.note.includes('from "QUALITATIVE ASSAY (NEGATIVE)" to "negative"'));

      const glucoseMeasurement = report.measurements.find((measurement) => measurement.canonicalCode === "fasting_glucose");
      assert.ok(glucoseMeasurement);
      assert.equal(glucoseMeasurement.textValue, "non-reactive");
      assert.ok(glucoseMeasurement.note.includes('from "REPEAT SCREEN (NON REACTIVE)" to "non-reactive"'));
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T12:00:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T12:00:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T12:00:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-colon-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional colon-label panel",
        observedAt: "2026-04-04T12:05:00.000Z",
        entries: [
          { name: "CRP", textValue: "SCREENING: POSITIVE" },
          { name: "Apolipoprotein B", textValue: "QUALITATIVE ASSAY: NEGATIVE" },
          { name: "Glucose", textValue: "REPEAT SCREEN: NON REACTIVE" },
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T12:05:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T12:05:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T12:05:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-comma-labeled-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional comma-label panel",
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T12:10:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T12:10:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T12:10:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-suffix-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional suffix-context panel",
        observedAt: "2026-04-04T12:15:00.000Z",
        entries: [
          { name: "CRP", textValue: "POSITIVE SCREENING RESULT" },
          { name: "Apolipoprotein B", textValue: "NEGATIVE QUALITATIVE ASSAY" },
          { name: "Glucose", textValue: "NON REACTIVE REPEAT SCREEN" },
        ],
      });
      ...
      assert.ok(glucoseMeasurement);
      assert.equal(glucoseMeasurement.textValue, "non-reactive");
      assert.ok(glucoseMeasurement.note.includes('from "NON REACTIVE REPEAT SCREEN" to "non-reactive"'));
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T12:15:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T12:15:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T12:15:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-base-labeled-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional base-label panel",
        observedAt: "2026-04-04T12:20:00.000Z",
        entries: [
          { name: "CRP", textValue: "POSITIVE: SCREENING RESULT" },
          { name: "Apolipoprotein B", textValue: "NEGATIVE, QUALITATIVE ASSAY" },
          { name: "Glucose", textValue: "NON REACTIVE = RESULT STATUS" },
        ],
      });
      ...
      assert.ok(glucoseMeasurement);
      assert.equal(glucoseMeasurement.textValue, "non-reactive");
      assert.ok(glucoseMeasurement.note.includes('from "NON REACTIVE = RESULT STATUS" to "non-reactive"'));
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T12:20:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T12:20:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T12:20:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-semicolon-separated-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional semicolon-separator panel",
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T12:30:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T12:30:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T12:30:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "report-intake-normalizes-pipe-separated-process-wording",
    covers: ["POST /api/intake/report"],
    coverageType: "success",
    async run() {
      const before = await getPatientSnapshot();
      const report = await postJson("/api/intake/report", {
        patientId,
        vendor: "Functional pipe-separator panel",
        observedAt: "2026-04-04T12:35:00.000Z",
        entries: [
          { name: "CRP", textValue: "SCREENING RESULT | POSITIVE" },
          { name: "Apolipoprotein B", textValue: "NEGATIVE | QUALITATIVE ASSAY" },
          { name: "Glucose", textValue: "RESULT STATUS | NON REACTIVE" },
      ...
      const persistedCrpMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "inflammation_crp" && measurement.observedAt === "2026-04-04T12:35:00.000Z",
      );
      assert.ok(persistedCrpMeasurement);
      assert.ok(persistedCrpMeasurement.interpretation.includes("text or categorical result positive"));

      const persistedApobMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "apob" && measurement.observedAt === "2026-04-04T12:35:00.000Z",
      );
      assert.ok(persistedApobMeasurement);
      assert.ok(persistedApobMeasurement.interpretation.includes("text or categorical result negative"));

      const persistedGlucoseMeasurement = after.patient.measurements.find(
        (measurement) => measurement.canonicalCode === "fasting_glucose" && measurement.observedAt === "2026-04-04T12:35:00.000Z",
      );
      assert.ok(persistedGlucoseMeasurement);
      assert.ok(persistedGlucoseMeasurement.interpretation.includes("text or categorical result non-reactive"));
    },
  ...
  {
    name: "promotion-normalizes-reviewed-prefix-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional prefix-context observation",
        observedAt: "2026-04-05T11:50:00.000Z",
        file: new File([createPrefixProcessTextFhirResource()], "functional-prefix-context-text-observation.json", {
          type: "application/json",
        }),
      });
  ...
  {
    name: "promotion-normalizes-reviewed-result-labeled-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional result-labeled observation",
        observedAt: "2026-04-05T11:55:00.000Z",
        file: new File([createResultLabeledProcessTextFhirResource()], "functional-result-labeled-text-observation.json", {
          type: "application/json",
        }),
      });
  ...
  {
    name: "promotion-normalizes-reviewed-parenthetical-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional parenthetical observation",
        observedAt: "2026-04-05T12:00:00.000Z",
        file: new File([createParentheticalProcessTextFhirResource()], "functional-parenthetical-text-observation.json", {
          type: "application/json",
        }),
      });
  ...
  {
    name: "promotion-normalizes-reviewed-colon-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional colon-label observation",
        observedAt: "2026-04-05T12:05:00.000Z",
        file: new File([createColonProcessTextFhirResource()], "functional-colon-label-text-observation.json", {
          type: "application/json",
        }),
      });
  ...
  {
    name: "promotion-normalizes-reviewed-comma-labeled-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional comma-label observation",
        observedAt: "2026-04-05T12:10:00.000Z",
        file: new File([createCommaLabeledProcessTextFhirResource()], "functional-comma-label-text-observation.json", {
          type: "application/json",
        }),
      });
  ...
  {
    name: "promotion-normalizes-reviewed-suffix-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional suffix observation",
        observedAt: "2026-04-05T12:15:00.000Z",
        file: new File([createSuffixProcessTextFhirResource()], "functional-suffix-text-observation.json", {
          type: "application/json",
        }),
      });
  ...
  {
    name: "promotion-normalizes-reviewed-base-labeled-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
  ...
  {
    name: "promotion-normalizes-reviewed-semicolon-separated-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional semicolon-separator observation",
        observedAt: "2026-04-05T12:30:00.000Z",
        file: new File([createSemicolonSeparatedProcessTextFhirResource()], "functional-semicolon-separator-text-observation.json", {
          type: "application/json",
        }),
      });
  ...
  {
    name: "promotion-normalizes-reviewed-pipe-separated-process-values",
    covers: ["POST /api/review/promote"],
    coverageType: "success",
    async run() {
      const upload = await postMultipart("/api/intake/document", {
        patientId,
        sourceSystem: "Functional pipe-separator observation",
        observedAt: "2026-04-05T12:35:00.000Z",
        file: new File([createPipeSeparatedProcessTextFhirResource()], "functional-pipe-separator-text-observation.json", {
          type: "application/json",
        }),
      });
